...
import edu.univalle.mobsim.qsim.pt.AbstractTransitDriverAgent;
import edu.univalle.mobsim.qsim.pt.DefaultTransitDriverAgentFactory;
import edu.univalle.mobsim.qsim.pt.PTPassengerAgent;
import edu.univalle.mobsim.qsim.pt.SimpleTransitStopHandlerFactory;
import edu.univalle.mobsim.qsim.pt.TransitDriverAgentFactory;
import edu.univalle.mobsim.qsim.pt.TransitQSimEngine;
import edu.univalle.mobsim.qsim.pt.TransitQVehicle;
import edu.univalle.mobsim.qsim.pt.TransitStopAgentTracker;
import edu.univalle.mobsim.qsim.pt.TransitStopHandlerFactory;
import edu.univalle.mobsim.qsim.pt.UmlaufCache;
...
public class TransitQSimEngine implements DepartureHandler, MobsimEngine, AgentSource
{
    ...
    public void afterSim() {
        ...
        for (Entry<Id<TransitStopFacility>, List<PTPassengerAgent>> agentsAtStop : this.agentTracker.getAgentsAtStop()
                .entrySet()) {
            ...
            for (PTPassengerAgent agent : agentsAtStop.getValue()) {
                this.qSim.getEventsManager().processEvent(
                        new PersonStuckEvent(now, agent.getId(), stop.getLinkId(), ((MobsimAgent) agent).getMode()));
    ...
    private static UmlaufCache getOrCreateUmlaufCache(final Scenario scenario) {
        ...
        ReconstructingUmlaufBuilder reconstructingUmlaufBuilder = new ReconstructingUmlaufBuilder(scenario.getNetwork(),
                scenario.getTransitSchedule().getTransitLines().values(), scenario.getTransitVehicles(),
    ...
    private void handleAgentPTDeparture(final MobsimAgent planAgent, Id<Link> linkId) {
        ...
        if (accessStopId == null) {
            ...
            log.error("pt-agent doesn't know to what transit stop to go. Removing agent from simulation. Agent "
        ...
        if (stop.getLinkId() == null || stop.getLinkId().equals(linkId)) {
            ...
            throw new TransitAgentTriesToTeleportException(
                    "Agent " + planAgent.getId() + " tries to enter a transit stop at link " + stop.getLinkId()
                            + " but really is at " + linkId + "!");
        }
    }
