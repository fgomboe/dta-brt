...
import edu.univalle.mobsim.qsim.pt.PTPassengerAgent;
import edu.univalle.mobsim.qsim.pt.PassengerAccessEgress;
import edu.univalle.mobsim.qsim.pt.TransitStopAgentTracker;
import edu.univalle.mobsim.qsim.pt.TransitStopHandler;
import edu.univalle.mobsim.qsim.pt.TransitVehicle;
...
import edu.univalle.mobsim.qsim.agents.PersonDriverAgentImpl;
...
class PassengerAccessEgressImpl implements PassengerAccessEgress
{
    ...
    PassengerAccessEgressImpl(InternalInterface internalInterface, TransitStopAgentTracker agentTracker,
        ...
        this.isGeneratingDeniedBoardingEvents = this.scenario.getConfig().vspExperimental()
                .isGeneratingBoardingDeniedEvents();
     ...
     */
    /*package*/ double calculateStopTimeAndTriggerBoarding(TransitRoute transitRoute, TransitLine transitLine,
            final TransitVehicle vehicle, final TransitStopFacility stop, List<TransitRouteStop> stopsToCome,
            final double now) {
        ...
        List<PTPassengerAgent> passengersEntering = findPassengersEntering(transitRoute, transitLine, vehicle, stop,
                stopsToCome, freeCapacity, now);
    ...
    private List<PTPassengerAgent> findPassengersEntering(TransitRoute transitRoute, TransitLine transitLine,
            TransitVehicle vehicle, final TransitStopFacility stop, List<TransitRouteStop> stopsToCome,
            int freeCapacity, double now) {
    ...
    private ArrayList<PTPassengerAgent> findPassengersLeaving(TransitVehicle vehicle, final TransitStopFacility stop) {
    ...
    @Override
    public boolean handlePassengerEntering(PTPassengerAgent passenger, MobsimVehicle vehicle,
            Id<TransitStopFacility> fromStopFacilityId, double time) {
    ...
    @Override
    public boolean handlePassengerLeaving(PTPassengerAgent passenger, MobsimVehicle vehicle, Id<Link> toLinkId,
            double time) {
        boolean handled = vehicle.removePassenger(passenger);
        if (handled) {
            eventsManager
                    .processEvent(new PersonLeavesVehicleEvent(time, passenger.getId(), vehicle.getVehicle().getId()));
