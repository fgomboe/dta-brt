...
import edu.univalle.mobsim.framework.PlanAgent;
...
public abstract class AbstractTransitDriverAgent implements TransitDriverAgent, PlanAgent
{
    ...
    AbstractTransitDriverAgent(InternalInterface internalInterface, TransitStopAgentTracker agentTracker2) {
        ...
        accessEgress = new PassengerAccessEgressImpl(this.internalInterface, agentTracker2, this.scenario,
                eventsManager);
    }
    ...
    public double handleTransitStop(final TransitStopFacility stop, final double now) {
        ...
        List<TransitRouteStop> stopsToCome = route.getStops().subList(stopIterator.nextIndex(),
                route.getStops().size());
        /*
         ...
         */
        double stopTime = this.accessEgress.calculateStopTimeAndTriggerBoarding(getTransitRoute(), getTransitLine(),
        ...
        if (internalInterface != null) {
            ...
            if (getTransitLine() == null) {
                eventsManager.processEvent(new TransitDriverStartsEvent(now, this.dummyPerson.getId(),
                        this.vehicle.getId(), Id.create("Wenden", TransitLine.class),
                        Id.create("Wenden", TransitRoute.class), Id.create("Wenden", Departure.class)));
            }
            else {
                eventsManager
                        .processEvent(new TransitDriverStartsEvent(now, this.dummyPerson.getId(), this.vehicle.getId(),
                                getTransitLine().getId(), getTransitRoute().getId(), getDeparture().getId()));
            }
        }
    }
    ...
    private void processEventVehicleArrives(final TransitStopFacility stop, final double now) {
        if (this.currentStop == null) {
            ...
            else if (!(Double.isNaN(this.currentStop.getDepartureOffset())
                    && Double.isInfinite(this.currentStop.getDepartureOffset()))) {
            ...
            eventsManager.processEvent(
                    new VehicleArrivesAtFacilityEvent(now, this.vehicle.getVehicle().getId(), stop.getId(), delay));
        }
    }
    ...
    private void depart(final double now) {
        ...
        eventsManager.processEvent(new VehicleDepartsAtFacilityEvent(now, this.vehicle.getVehicle().getId(),
                this.currentStop.getStopFacility().getId(), delay));
        this.nextStop = (stopIterator.hasNext() ? stopIterator.next() : null);
        if (this.nextStop == null) {
    ...
    private void assertVehicleIsEmpty() {
        if (this.vehicle.getPassengers().size() > 0) {
            ...
            log.error(
                    "Transit vehicle must be empty after last stop! vehicle-id = " + this.vehicle.getVehicle().getId(),
                    e);
            for (PassengerAgent agent : this.vehicle.getPassengers()) {
                if (agent instanceof PersonDriverAgentImpl) {
                    log.error("Agent is still in transit vehicle: agent-id = "
