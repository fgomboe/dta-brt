...
public class QNode implements NetsimNode
{
    ...
    private boolean checkNextLinkSemantics(Link currentLink, Id<Link> nextLinkId, QLinkInternalI nextQLink,
            QVehicle veh) {
        if (nextQLink == null) {
        ...
        if (currentLink.getToNode() != nextQLink.getLink().getFromNode()) {
            ...
            log.warn("Cannot move vehicle " + veh.getId() + " from link " + currentLink.getId() + " to link "
    ...
    private static void moveVehicleFromInlinkToOutlink(final QVehicle veh, final QInternalI fromLane,
            QLinkInternalI nextQueueLink) {
        fromLane.popFirstVehicle();
