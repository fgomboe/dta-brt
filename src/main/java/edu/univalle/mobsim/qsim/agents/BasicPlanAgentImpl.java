package edu.univalle.mobsim.qsim.agents;
...
import edu.univalle.mobsim.framework.HasPerson;
...
import edu.univalle.mobsim.framework.VehicleUsingAgent;
...
public final class BasicPlanAgentImpl
        implements MobsimAgent, PlanAgent, Identifiable<Person>, HasPerson, VehicleUsingAgent
{
    ...
    public final void endLegAndComputeNextState(final double now) {
        this.getEvents().processEvent(
                new PersonArrivalEvent(now, this.getId(), this.getDestinationLinkId(), getCurrentLeg().getMode()));
    ...
    private void advancePlan(double now) {
        ...
        if (this.getCurrentPlanElementIndex() >= this.getCurrentPlan().getPlanElements().size()) {
            log.error("plan of agent with id = " + this.getId()
    ...
    private void initializeLeg(Leg leg) {
        ...
        if (leg.getRoute() == null) {
            log.error("The agent " + this.getPerson().getId()
    ...
    private final void calculateAndSetDepartureTime(Activity act) {
        PlansConfigGroup.ActivityDurationInterpretation activityDurationInterpretation = this.getScenario().getConfig()
                .plans().getActivityDurationInterpretation();
        ...
        if (this.getCurrentPlanElementIndex() == this.getCurrentPlan().getPlanElements().size() - 1) {
            if (finalActHasDpTimeWrnCnt < 1 && departure != Double.POSITIVE_INFINITY) {
                log.error("last activity of person driver agent id " + this.getId()
    ...
    public final void endActivityAndComputeNextState(final double now) {
        ...
        this.getEvents().processEvent(new ActivityEndEvent(now, this.getPerson().getId(), act.getLinkId(),
