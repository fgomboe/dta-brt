...
import edu.univalle.mobsim.jdeqsim.DeadlockPreventionMessage;
import edu.univalle.mobsim.jdeqsim.Road;
import edu.univalle.mobsim.jdeqsim.Scheduler;
import edu.univalle.mobsim.jdeqsim.SimUnit;
...
public class Road extends SimUnit
{
    ...
    public Road(Scheduler scheduler, Link link) {
         ...
         * same time
         */
        this.maxNumberOfCarsOnRoad = Math
                .round(link.getLength() * NetworkUtils.getNumberOfLanesAsInt(Time.UNDEFINED_TIME, link)
        ...
        double maxInverseInFlowCapacity = 3600
                / (SimulationParameters.getMinimumInFlowCapacity() * SimulationParameters.getFlowCapacityFactor()
                        * NetworkUtils.getNumberOfLanesAsInt(Time.UNDEFINED_TIME, link));

        this.inverseOutFlowCapacity = 1
                / (((LinkImpl) link).getFlowCapacity() * SimulationParameters.getFlowCapacityFactor());
    ...
    public void leaveRoad(Vehicle vehicle, double simTime) {
        ...
        if (this.interestedInEnteringRoad.size() > 0) {
            ...
            double nextAvailableTimeForEnteringStreet = Math
                    .max(this.timeOfLastEnteringVehicle + this.inverseInFlowCapacity, simTime + this.gapTravelTime);
    ...
    public void enterRequest(Vehicle vehicle, double simTime) {
        ...
        if (this.carsOnTheRoad.size() + this.noOfCarsPromisedToEnterRoad < this.maxNumberOfCarsOnRoad) {
            ...
            double nextAvailableTimeForEnteringStreet = Math.max(
                    Math.max(this.timeOfLastEnteringVehicle + this.inverseInFlowCapacity, simTime), arrivalTimeOfGap);
            ...
            if (this.deadlockPreventionMessages.size() > 0) {
                ...
                this.deadlockPreventionMessages.add(vehicle
                        .scheduleDeadlockPreventionMessage(simTime + SimulationParameters.getSqueezeTime(), this));
            }

            assert (this.interestedInEnteringRoad.size() == this.deadlockPreventionMessages
                    .size()) : this.interestedInEnteringRoad.size() + " - " + this.deadlockPreventionMessages.size();
        }
    }
    ...
    public void removeFirstDeadlockPreventionMessage(DeadlockPreventionMessage dpMessage) {
        ...
        assert (this.deadlockPreventionMessages
                .getFirst() == dpMessage) : "Inconsitency in logic!!! => this should only be invoked from the handler of this message";
