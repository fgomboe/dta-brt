...
import edu.univalle.mobsim.qsim.agents.ActivityDurationUtils;
...
public class EndLegMessage extends EventMessage
{
    ...
    public void handleMessage() {
        ...
        if ((actsLegs.size() > this.vehicle.getLegIndex())) {
            ...
            double departureTime = ActivityDurationUtils.calculateDepartureTime(currentAct, getMessageArrivalTime(),
    ...
    public void processEvent() {
        ...
        Id<org.matsim.vehicles.Vehicle> vehicleId = Id.create(this.vehicle.getOwnerPerson().getId(),
                org.matsim.vehicles.Vehicle.class);
        event = new VehicleLeavesTrafficEvent(this.getMessageArrivalTime(), this.vehicle.getOwnerPerson().getId(),
                this.vehicle.getCurrentLinkId(), vehicleId, this.vehicle.getCurrentLeg().getMode(), 1.0);
        SimulationParameters.getProcessEventThread().processEvent(event);

        // schedule AgentArrivalEvent
        event = new PersonArrivalEvent(this.getMessageArrivalTime(), this.vehicle.getOwnerPerson().getId(),
                this.vehicle.getCurrentLinkId(), this.vehicle.getCurrentLeg().getMode());
        SimulationParameters.getProcessEventThread().processEvent(event);
        ...
        event = new ActivityStartEvent(actStartEventTime, this.vehicle.getOwnerPerson().getId(),
                this.vehicle.getCurrentLinkId(), nextAct.getFacilityId(), nextAct.getType());
        SimulationParameters.getProcessEventThread().processEvent(event);
