...
public final class QLinkImpl extends AbstractQLink implements SignalizeableItem
{
    ...
    final void addFromUpstream(final QVehicle veh) {
        ...
        this.network.simEngine.getMobsim().getEventsManager()
                .processEvent(new LinkEnterEvent(now, veh.getDriver().getId(), this.link.getId(), veh.getId()));
    ...
    private boolean checkForActivity() {
         ...
         * is not delayed).
         */
        return road.isActive() || !this.waitingList.isEmpty()
    ...
    class VisDataImpl implements VisData
    {
        ...
        private VisDataImpl() {
            double nodeOffset = QLinkImpl.this.network.simEngine.getMobsim().getScenario().getConfig().qsim()
                    .getNodeOffset();
            if (nodeOffset != 0.0) {
        ...
        public Collection<AgentSnapshotInfo> addAgentSnapshotInfo(Collection<AgentSnapshotInfo> positions) {
            AbstractAgentSnapshotInfoBuilder snapshotInfoBuilder = QLinkImpl.this.network.simEngine
                    .getAgentSnapshotInfoBuilder();

            VisData roadVisData = road.getVisData();
            if (visLink != null) {
                ((QueueWithBuffer.VisDataImpl) roadVisData).setVisInfo(visLink.getLinkStartCoord(),
                        visLink.getLinkEndCoord(), visLink.getEuklideanDistance());
