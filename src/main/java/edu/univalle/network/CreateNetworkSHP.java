...
public class CreateNetworkSHP {
    ...
    public static void main(String[] args) throws Exception {
        ...
        config.network().setInputFile("output/specialNetwork.xml");
        ...
        new ObjectAttributesXmlReader(linkAttributes).parse("./output/specialLinkAttributes.xml");
        new ObjectAttributesXmlReader(nodeAttributes).parse("./output/specialNodeAttributes.xml");
        ...
        PointFeatureFactory nodeFactory = new PointFeatureFactory.Builder().
                ...
                addAttribute("shortName", String.class).
                addAttribute("longName", String.class).
                addAttribute("gpsX", Double.class).
                addAttribute("gpsY", Double.class).
                addAttribute("lines", String.class).
                addAttribute("inLines", String.class).
                addAttribute("outLines", String.class).
                create();

        TreeMap<String, Integer> nodeNames = new TreeMap<String, Integer>();
        int id = 1;
        for (Node node : network.getNodes().values()) {
        	nodeNames.put(node.getId().toString(), id);
        	String longName = node.getId().toString().length() <= 2 ? 
        						(String) nodeAttributes.getAttribute(node.getId().toString(), "ESTACION") : 
        						(String) nodeAttributes.getAttribute(node.getId().toString(), "LONGNAME");
        	Double gpsX = node.getId().toString().length() <= 2 ? 
	        				(Double) nodeAttributes.getAttribute(node.getId().toString(), "GPS_X") : 
	        				(Double) nodeAttributes.getAttribute(node.getId().toString(), "LONGITUDE");
        	Double gpsY = node.getId().toString().length() <= 2 ? 
	        				(Double) nodeAttributes.getAttribute(node.getId().toString(), "GPS_Y") : 
	        				(Double) nodeAttributes.getAttribute(node.getId().toString(), "LATITUDE");
            ...
            SimpleFeature ft = nodeFactory.createPoint(node.getCoord(), new Object[] {id, node.getId().toString(), longName, gpsX, gpsY, lines != null ? lines.substring(1, lines.length() - 1) : lines, inLines != null ? inLines.substring(1, inLines.length() - 1) : inLines, outLines != null ? outLines.substring(1, outLines.length() - 1) : outLines}, null);
        ...
        ShapeFileWriter.writeGeometries(features, "input/shapes/Cali_special_generated_trunk_network/network_nodes.shp");
...
//                addAttribute("toNode", String.class).
                addAttribute("toNode", Integer.class).
                addAttribute("length", Double.class).
                addAttribute("freespeed", Double.class).
                addAttribute("capacity", Double.class).
                addAttribute("noOfLanes", Integer.class).
                addAttribute("lines", String.class).
                addAttribute("line_route", String.class).
                create();
...
//                    new Object [] {id, link.getId().toString(), link.getFromNode().getId().toString(), link.getToNode().getId().toString(), link.getLength(), link.getFreespeed(), link.getCapacity(), link.getNumberOfLanes(), linesAtt.substring(1, linesAtt.length() - 1), linesRoutesAtt.substring(1, linesRoutesAtt.length() - 1)}, null);
            		new Object [] {id, link.getId().toString(), nodeNames.get(fromNode), nodeNames.get(toNode), link.getLength(), link.getFreespeed(), link.getCapacity(), link.getNumberOfLanes(), linesAtt.substring(1, linesAtt.length() - 1), linesRoutesAtt.substring(1, linesRoutesAtt.length() - 1)}, null);
         // Comment the following 3 lines to export entire network
            List<String> linesArray = new ArrayList<String>();
        ...
        ShapeFileWriter.writeGeometries(features, "input/shapes/Cali_special_generated_trunk_network/network_links.shp");
    }
}