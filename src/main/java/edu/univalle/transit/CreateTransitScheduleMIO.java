            ...
            while (arcos.readRecord()) {
                ...
                if (line.equals("T31") || line.equals("E31") || line.equals("E21") || currentLine.equals("T31")) {
                    ...
                    if (!line.equals(currentLine)) {
                        if (currentLine != "") schedule.addTransitLine(tLine);
        ...
        try {
            ...
            for (String linea : lineas) {
                ...
                while (salidas.readRecord()) {
                    ...
                    Departure ida = schedule.getFactory().createDeparture(Id.create(id_dep, Departure.class),
                            Double.parseDouble(salidas.get("Hora_mat")));
                    ida.setVehicleId(Id.create(salidas.get("Veh_ida"), org.matsim.vehicles.Vehicle.class));
                    schedule.getTransitLines().get(lineId).getRoutes().get(Id.create("Ida", TransitRoute.class))
                            .addDeparture(ida);

                    Departure regreso = schedule.getFactory().createDeparture(Id.create(id_dep, Departure.class),
                            Double.parseDouble(salidas.get("Hora_mat")));
                    regreso.setVehicleId(Id.create(salidas.get("Veh_regreso"), org.matsim.vehicles.Vehicle.class));
                    schedule.getTransitLines().get(lineId).getRoutes().get(Id.create("Vuelta", TransitRoute.class))
                            .addDeparture(regreso);
