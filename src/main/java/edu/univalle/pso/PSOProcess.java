...
public class PSOProcess implements PSOConstants
{
    ...
    private static Multihilo[] list_multi = new Multihilo[SWARM_SIZE];
    ...
    public void execute() {
        ...
        while (t < MAX_ITERATION && err > ProblemSet.ERR_TOLERANCE) {
            ...
            /**
             * Here should go something like:
             * err = ProblemSet.evaluate(gBestLocation) - GEH_preferred (ideally 5%)
             **/
    ...
    public void updateFitnessList() {
        for (int i = 0; i < SWARM_SIZE; i++) {
            ...
            // Use this for no parallel execution
            // fitnessValueList[i] = swarm.get(i).getFitnessValue(); }

            // Use this for parallel execution
            /**
             * Here should go something like:
             * worst_geh[i]=matsim_funct (calibration parameters)
             **/
            list_multi[i] = new Multihilo(i, swarm.get(i).getLocation());
            list_multi[i].start();

        }
        try {
            for (int i = 0; i < SWARM_SIZE; i++) {
                list_multi[i].hilo.join();
            }
        ...
        fitnessValueList = Multihilo.Resultados2;
