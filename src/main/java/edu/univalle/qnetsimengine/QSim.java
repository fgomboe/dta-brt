 ...
 * approach consists of the following elements (and presumably more, developed by mzilske):
 * <ul>
...
public final class QSim extends Thread implements VisMobsim, Netsim, ActivityEndRescheduler
{
    ...
    private final edu.univalle.qnetsimengine.AgentCounter agentCounter;
    ...
    public QSim(final Scenario sc, EventsManager events) {
        ...
        this.agentCounter = new edu.univalle.qnetsimengine.AgentCounter();
    ...
    /*package*/ void prepareSim() {
        ...
        createAgents();
        this.initSimTimer();
        this.infoTime = Math.floor(this.simTimer.getSimStartTime() / INFO_PERIOD) * INFO_PERIOD; // infoTime may be < simStartTime, this ensures
    ...
    /*package*/ boolean doSimStep() {
        ...
        if (withindayEngine != null)
            withindayEngine.doSimStep(time);
    ...
    private void arrangeNextAgentAction(MobsimAgent agent) {
        ...
        break;
        case ABORT:
            this.events.processEvent(new PersonStuckEvent(this.simTimer.getTimeOfDay(), agent.getId(),
                    agent.getCurrentLinkId(), agent.getMode()));
    ...
    private void initSimTimer() {
        ...
        double simStartTime;
        if (QSimConfigGroup.StarttimeInterpretation.maxOfStarttimeAndEarliestActivityEnd
        ...
        else if (QSimConfigGroup.StarttimeInterpretation.onlyUseStarttime
                .equals(qSimConfigGroup.getSimStarttimeInterpretation())) {
    ...
    private void printSimLog(final double time) {
        if (time >= this.infoTime) {
            ...
            log.info("SIMULATION (NEW QSim) AT " + Time.writeTime(time) + " : #Veh=" + this.agentCounter.getLiving()
                    + " lost=" + this.agentCounter.getLost() + " simT=" + diffsim + "s realT=" + (diffreal)
                    + "s; (s/r): " + (diffsim / (diffreal + Double.MIN_VALUE)));
