...
import edu.univalle.qnetsimengine.QueueWithBuffer.Hole;
...
abstract class AbstractAgentSnapshotInfoBuilder
{
     ...
     */
    public final int positionVehiclesFromWaitingList(final Collection<AgentSnapshotInfo> positions, final Link link,
            int cnt2, final Queue<QVehicle> waitingList) {
        for (QVehicle veh : waitingList) {
            ...
            for (Identifiable passenger : peopleInVehicle) {
                ...
                AgentSnapshotInfo passengerPosition = snapshotInfoFactory.createAgentSnapshotInfo(passenger.getId(),
                        link, 0.9 * link.getLength(), cnt2); // for the time being, same position as facilities
    ...
    public final int positionAgentsInActivities(final Collection<AgentSnapshotInfo> positions, Link link,
        ...
        for (MobsimAgent pa : agentsInActivities) {
            AgentSnapshotInfo agInfo = snapshotInfoFactory.createAgentSnapshotInfo(pa.getId(), link,
                    0.9 * link.getLength(), cnt2);
    ...
    public final int positionVehiclesFromTransitStop(final Collection<AgentSnapshotInfo> positions, Link link,
        ...
        if (transitVehicleStopQueue.size() > 0) {
            for (QVehicle veh : transitVehicleStopQueue) {
                ...
                for (ListIterator<Identifiable> it = peopleInVehicle.listIterator(peopleInVehicle.size()); it
                        .hasPrevious();) {
    ...
    public final void positionAgentOnLink(final Collection<AgentSnapshotInfo> positions, Coord startCoord,
        ...
        if (scenario.getPopulation().getPersonAttributes().getAttribute(driverAgent.getId().toString(),
                "marker") != null) {
            pos.setAgentState(AgentState.PERSON_OTHER_MODE);
        }

        this.positionPassengers(positions, veh.getPassengers(), distanceFromFromNode, startCoord, endCoord,
                lengthOfCurve, euclideanLength, lane + 5, speedValueBetweenZeroAndOne);
        // (this is deliberately first memorizing "pos" but then filling in the passengers first)

        positions.add(pos);
    }

    public final void positionVehiclesAlongLine(Collection<AgentSnapshotInfo> positions, double now,
            Collection<MobsimVehicle> vehs, TreeMap<Double, Hole> holePositions, double curvedLength,
        ...
        for (MobsimVehicle mveh : vehs) {
            ...
            this.positionAgentOnLink(positions, upstreamCoord, downstreamCoord, curvedLength, euklideanDistance, veh,
    ...
    public final void positionQItem(final Collection<AgentSnapshotInfo> positions, Coord startCoord, Coord endCoord,
            double lengthOfCurve, double euclideanLength, QItem veh, double distanceFromFromNode, Integer lane,
            double speedValueBetweenZeroAndOne) {
        AgentSnapshotInfo pos = snapshotInfoFactory.createAgentSnapshotInfo(Id.create("hole", Person.class), endCoord,
                startCoord, distanceFromFromNode, lane, lengthOfCurve, euclideanLength);
        pos.setColorValueBetweenZeroAndOne(speedValueBetweenZeroAndOne);
        pos.setAgentState(AgentState.PERSON_OTHER_MODE);
        positions.add(pos);
    }

    public final static double calcSpeedValueBetweenZeroAndOne(QVehicle veh, double inverseSimulatedFlowCapacity,
            double now, double freespeed) {
    ...
    final void positionPassengers(Collection<AgentSnapshotInfo> positions,
        ...
        for (PassengerAgent passenger : passengers) {
            ...
            AgentSnapshotInfo passengerPosition = snapshotInfoFactory.createAgentSnapshotInfo(passenger.getId(),
                    startCoord, endCoord, distanceOnLink, lanePos, lengthOfCurve, euclideanLength);
    ...
    public abstract double calculateVehicleSpacing(double linkLength, double numberOfVehiclesOnLink,
     ...
     * @return
     */
    public abstract double calculateDistanceOnVectorFromFromNode2(double length, double spacing,
