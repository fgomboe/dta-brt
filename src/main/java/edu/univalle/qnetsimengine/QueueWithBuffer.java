...
import edu.univalle.qnetsimengine.AbstractQLink.HandleTransitStopResult;
...
final class QueueWithBuffer extends QLaneI implements SignalizeableItem
{
    ...
    private QueueWithBuffer(AbstractQLink qLinkImpl, final VehicleQ<QVehicle> vehicleQueue, Id<Lane> id, double length,
            double effectiveNumberOfLanes, double flowCapacity_s) {
        this.id = id;
    ...
    private boolean hasFlowCapacityLeftAndBufferSpace() {
        ...
        if (fastCapacityUpdate) {
            updateFlowAccumulation(now);
            return (usedBufferStorageCapacity < bufferStorageCapacity && ((flowcap_accumulate.getValue() >= 0.0)));
        }
        else {
    ...
    private void updateFlowAccumulation(final double now) {
        ...
        if (this.flowcap_accumulate.getTimeStep() < now && this.flowcap_accumulate.getValue() < 0
        ...
        while ((veh = peekFromVehQueue()) != null) {
            ...
            if (driver instanceof TransitDriverAgent) {
                HandleTransitStopResult handleTransitStop = qLink.transitQLink.handleTransitStop(now, veh,
                        (TransitDriverAgent) driver, this.qLink.link.getId());
            ...
            addToBuffer(veh, now);
            removeVehicleFromQueue(now, veh);
            if (isRestrictingSeepage && isSeepageAllowed && veh.getDriver().getMode().equals(seepMode))
    ...
    private QVehicle removeVehicleFromQueue(final double now, final QVehicle veh2Remove) {
        ...
        if (isSeepageAllowed && isSeepModeStorageFree
                && veh.getVehicle().getType().getId().toString().equals(seepMode)) {
    ...
    public final QVehicle popFirstVehicle() {
        ...
        if (this.generatingEvents) {
            this.network.simEngine.getMobsim().getEventsManager()
                    .processEvent(new LaneLeaveEvent(now, veh.getDriver().getId(), this.link.getId(), this.getId()));
        }
        network.simEngine.getMobsim().getEventsManager()
                .processEvent(new LinkLeaveEvent(now, veh.getDriver().getId(), this.link.getId(), veh.getId()));
        return veh;
    }
    ...
    public final void clearVehicles() {
        ...
        for (QVehicle veh : vehQueue) {
            ...
            network.simEngine.getMobsim().getEventsManager().processEvent(new PersonStuckEvent(now,
    ...
    public final void addFromUpstream(final QVehicle veh) {
        ...
        double linkTravelTime = this.length
                / this.network.simEngine.getLinkSpeedCalculator().getMaximumVelocity(veh, link, now);
    ...
    class VisDataImpl implements VisData
    {
        ...
        public final Collection<AgentSnapshotInfo> addAgentSnapshotInfo(Collection<AgentSnapshotInfo> positions) {
            ...
            if (VIS_HOLES) {
                // holes:
                if (!holes.isEmpty()) {
                    double spacing = snapshotInfoBuilder.calculateVehicleSpacing(length, holes.size(),
                            getStorageCapacity());
                    double freespeedTraveltime = length / (hole_speed * 1000. / 3600.);
                    double lastDistanceFromFromNode = Double.NaN;
                    for (Hole hole : holes) {
                        lastDistanceFromFromNode = createHolePositionAndReturnDistance(snapshotInfoBuilder, now,
                                lastDistanceFromFromNode, spacing, freespeedTraveltime, hole);
                        if (VIS_HOLES) {
            ...
            if (!buffer.isEmpty() || !vehQueue.isEmpty()) {
                ...
                snapshotInfoBuilder.positionVehiclesAlongLine(positions, now, getAllVehicles(), holePositions, length,
                        storageCapacity + bufferStorageCapacity, ((LinkImpl) link).getEuklideanDistance(),
                        link.getFromNode().getCoord(), link.getToNode().getCoord(), inverseFlowCapacityPerTimeStep,
        ...
        private double createHolePositionAndReturnDistance(AbstractAgentSnapshotInfoBuilder snapshotInfoBuilder,
                double now, double lastDistanceFromFromNode, double spacing, double freespeedTraveltime, Hole veh) {
            double remainingTravelTime = veh.getEarliestLinkExitTime() - now;
            double distanceFromFromNode = snapshotInfoBuilder.calculateDistanceOnVectorFromFromNode2(
                    QueueWithBuffer.this.length, spacing, lastDistanceFromFromNode, now, freespeedTraveltime,
                    remainingTravelTime);
        ...
        private void addHolePosition(final Collection<AgentSnapshotInfo> positions,
            ...
            if (this.upstreamCoord != null) {
                snapshotInfoBuilder.positionQItem(positions, this.upstreamCoord, this.downsteamCoord,
                        QueueWithBuffer.this.length, this.euklideanDistance, veh, distanceFromFromNode, lane,
                        speedValue);
            }
            else {
