...
public class QNetsimEngine implements MobsimEngine
{
    ...
    private LinkSpeedCalculator linkSpeedCalculator = new UnivalleLinkSpeedCalculator();
    ...
    public QNetsimEngine(final QSim sim, NetsimNetworkFactory netsimNetworkFactory) {
        ...
        default:
            throw new RuntimeException(
                    "trafficDynamics defined in config that does not exist: " + qsimConfigGroup.getTrafficDynamics());
        }
        ...
        if (qSimConfigGroup.getLinkDynamics().equals(LinkDynamics.SeepageQ)) {
            ...
            if (QueueWithBuffer.isSeepModeStorageFree)
                log.warn("Seep mode " + QueueWithBuffer.seepMode
                        + " do not take storage space thus only considered for flow capacities.");
        ...
        if (config.qsim().isUseLanes()) {
            ...
            if (netsimNetworkFactory != null) {
                throw new RuntimeException(
                        "both `lanes' and `netsimNetworkFactory' are defined; don't know what that means; aborting");
            }
            network = new QNetwork(sim.getScenario().getNetwork(),
            ...
            network = new QNetwork(sim.getScenario().getNetwork(), netsimNetworkFactory);
                ...
                || qsimConfigGroup.getLinkDynamics() == QSimConfigGroup.LinkDynamics.SeepageQ) {
    ...
    public void addParkedVehicle(MobsimVehicle veh, Id<Link> startLinkId) {
        ...
        if (qlink == null) {
            throw new RuntimeException(
                    "requested link with id=" + startLinkId + " does not exist in network.  Possible vehicles "
    ...
    public void onPrepareSim() {
        this.infoTime = Math.floor(internalInterface.getMobsim().getSimTimer().getSimStartTime() / INFO_PERIOD)
                * INFO_PERIOD;
        /*
    ...
    /*package*/ void printSimLog(double time) {
        if (time >= this.infoTime) {
            ...
            log.info("SIMULATION (QNetsimEngine) AT " + Time.writeTime(time) + " : #links=" + nofActiveLinks
